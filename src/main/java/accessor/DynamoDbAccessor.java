...
import exception.DependencyFailureException;
...
public class DynamoDbAccessor {
    ...
    public void writeItems(@NonNull final List<Object> items) {
        try {
            final List<List<Object>> itemsBatchPartitions = Lists.partition(items, BATCH_SIZE);
            for (int batch_partition_idx = 0; batch_partition_idx < itemsBatchPartitions.size();
                 batch_partition_idx++) {
                dynamoDBMapper.batchSave(itemsBatchPartitions.get(batch_partition_idx));
            }
        } catch (final Exception e) {
            throw new DependencyFailureException(e);
        }
