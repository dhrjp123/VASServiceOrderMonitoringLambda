...
public class GetJobMetricsComponent {
    ...
    public GetJobMetricsOutputBO getJobMetrics(@NonNull final GetJobMetricsInputBO getJobMetricsInputBO){
        ...
        final Map<List<String>, JobAggregatedMetricsBO> jobMetricsMap = groupingFunctionJobDetailsMap
                .entrySet().stream()
                .collect(Collectors.toMap(e -> e.getKey() , e -> calculateJobAggregateMetrics(e.getValue())));
    ...
    private JobAggregatedMetricsBO calculateJobAggregateMetrics(final List<JobDetailsBO> jobDetailsBOList){
        int etaDelayCount = 0;
    ...
    private String getGroupingCriteriaValue(String criterion, JobDetailsBO jobDetailsBO){
        switch (criterion){
            case "city":return jobDetailsBO.getCity();
            case "merchantID":return jobDetailsBO.getMerchantID();
            case "technicianID": return jobDetailsBO.getTechnicianID();
            case "time" : return jobDetailsBO.getTime();
            default :return null;
        }
    }
